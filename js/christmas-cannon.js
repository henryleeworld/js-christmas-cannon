...
import {
    GLTFLoader
} from "https://cdn.skypack.dev/three@0.122.0/examples/jsm/loaders/GLTFLoader";
...
class Stage {
    constructor(scale, stageSize, onReady, showStats) {
        ...
        sofaLoader.load("models/sofa.glb", object => {
        ...
        treeLoader.load("models/pine-tree.gltf", object => {
        ...
        var tableLoader = new GLTFLoader(manager);
        tableLoader.load("models/cannon.gltf", object => {
    ...
    createBall(size, color, light = false) {
        ...
        if (!light) {
            ...
            let capMat = new MeshPhongMaterial({
                color: 0xFFD338,
                wireframe: false
            });
    ...
    createCylinder(radiusTop, radiusBottom, height, numSegments) {
        ...
        const geometry = new CylinderGeometry(radiusTop, radiusBottom, height, numSegments);
...
class Physics {
    constructor(scale = 1, stageSize) {
        ...
        this.materials[PHYSICS_MATERIAL.lowBounce] = new CANNON.ContactMaterial(mainMaterial, mainMaterial, {
            friction: 2,
            restitution: 0 + extraBounce
        });
        this.materials[PHYSICS_MATERIAL.normalBounce] = new CANNON.ContactMaterial(mainMaterial, mainMaterial, {
            friction: 2,
            restitution: 0 + extraBounce
        });
        this.materials[PHYSICS_MATERIAL.highBounce] = new CANNON.ContactMaterial(mainMaterial, mainMaterial, {
            friction: 2,
            restitution: 1.5 + extraBounce
        });
...
let sounds = {
    fire: [new Audio('sounds/fire-01.mp3'), new Audio('sounds/fire-02.mp3')],
    bells: [new Audio('sounds/bells-01.mp3')],
    bing: [new Audio('sounds/bing-01.mp3'), new Audio('sounds/bing-02.mp3')]
};
...
function introRoom() {
    ...
    let roomTL = gsap.timeline({
        onComplete: () => setState(GAME_STATE.waiting)
    });
    roomTL.to('#man', {
        delay: 1,
        y: 0,
        ...
        ease: 'power4.out',
        duration: 0.6,
        onComplete: () => playSound('bing')
    });
    roomTL.fromTo('#bubble', {
        autoAlpha: 0,
        y: 0,
        x: '+=50',
        rotation: 5,
...
function introCannon() {
    ...
    cannonTL.to(stage.camera.position, {
        x: 65,
        y: -20,
        z: 20
    });
    cannonTL.to(stage.cameraTarget, {
        x: 30,
        y: -25,
        z: 30
    }, 0);
    ...
    cannonTL.to(manEl, {
        ...
        duration: 0.1,
        onComplete: () => {
            manEl.setAttribute('src', 'images/man.svg');
            textEl.textContent = "但是等等，我們有這個";
            textHighlightEl.textContent = "聖誕節禮物炮！！";
            setTimeout(() => playSound('bing'), 300);

        }
    }, 0.5);
    cannonTL.fromTo('#bubble', {
        autoAlpha: 0,
        y: 0,
        x: '+=50',
        rotation: 5,
...
function introStart() {
    ...
    let cannonTL = gsap.timeline({
        onComplete: () => setState(GAME_STATE.game),
        defaults: {
            ease: 'power4.easeInOut',
            duration: .8
        }
    ...
    cannonTL.to(manEl, {
        ...
        onComplete: () => {
            manEl.setAttribute('src', 'images/man.svg');
            textEl.textContent = "點擊以觸發聖誕節禮物炮，讓這個房間更加充滿歡樂！";
...
function endMessage() {
    ...
    let changed = false;
    manEl.addEventListener('click', event => {
        event.stopPropagation();
        if (!changed) {
            changed = true;
            let manTL = gsap.timeline();
            manTL.to(manEl, {
                y: '-=40',
                duration: 0.1,
                onComplete: () => {
                    manEl.setAttribute('src', 'images/man-sweater.svg');
                }
            }, 0);
            manTL.to(manEl, {
                y: 0,
    ...
    textEl.innerHTML = `好多了。`;

    textHighlightEl.textContent = "聖誕節快樂！";
...
function startGame() {
    ...
    let gameStartTL = gsap.timeline({
        defaults: {
            ease: 'power4.easeInOut',
            duration: .5
        }
    });
    gameStartTL.to('#bubble', {
        autoAlpha: 0,
        y: '-=30',
        scale: 0.5,
        duration: 0.3,
...
function fire(coords) {
    ...
    if (stars.length && Math.random() > 0.8) item = createStar();
    else item = addBall();
...
function onReady() {
    ...
    cannonFlash.fromTo(stage.cannonLight, {
        intensity: 2
    }, {
        intensity: 0,
        duration: .3
    ...
    cannonRecoil.to(stage.models.cannon.position, {
        x: '+=3',
        z: '+=3',
        duration: 0.1,
        ease: 'Power2.out'
    }).to(stage.models.cannon.position, {
        x: '-=3',
        z: '-=3',
        duration: 0.4
    });
...
function createTree() {
    let body = physics.createBody(10, {
        x: 30,
        y: -10,
        z: 30
    }, {
